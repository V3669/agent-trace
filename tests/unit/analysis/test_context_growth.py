...
class TestComputeContextGrowth:
    ...
    def test_single_request_returns_one_point(
        self, tmp_db: tuple[Path, object]
    ) -> None:
        db_path, conn = tmp_db
        r1 = _insert_request(
    ...
    def test_points_ordered_by_recv_ts(
        self, tmp_db: tuple[Path, object]
    ) -> None:
        db_path, conn = tmp_db
        ...
        for i, r in enumerate([r1, r2, r3], 0):
            _insert_session_event(
                conn, session_id="sess-order", request_id=r,
    ...
    def test_plain_input_tokens_correct(
        self, tmp_db: tuple[Path, object]
    ) -> None:
        db_path, conn = tmp_db
    ...
    def test_unpriced_model_cost_is_zero(
        self, tmp_db: tuple[Path, object]
    ) -> None:
    ...
    def test_usage_source_propagated(
        self, tmp_db: tuple[Path, object]
    ) -> None:
        db_path, conn = tmp_db
        r1 = _insert_request(
